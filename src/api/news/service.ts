...
export const newsMock = [
    ...
    },
    {
        id: 3,
        title:
            'Заголовок новости 3, который очень длинный ' +
            'и занимает много места, прямо очень много, вообще ужас',
        text:
            'Текст новости 3 В CSS есть свойство box-sizing, которое определяет, как рассчитывается ширина и высота элемента' +
...
class NewsService {
    ...
    async getAllNews(): Promise<News[]> {
        const news = newsMock.map((news) => ({
            ...news,
            poster: news.pictureURL,
        }));

        return news.length > 0 ? Promise.resolve(news) : Promise.resolve([]);
    }
    ...
    getNewsById(id: number): Promise<News> {
        ...
        if (!news) {
            throw new Error('News not found'); // Use Error object for rejection
        }
